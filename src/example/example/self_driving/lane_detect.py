...
class LaneDetector(object):
    def __init__(self, color):
        ...
        if os.environ["DEPTH_CAMERA_TYPE"] == "ascamera":
            self.rois = (
                (368, 390, 0, 320, 0.7),  # 박스 1 (기존 1번)
                (345, 368, 0, 320, 0.0),  # TODO 01 : 박스 2 (새로 추가)
                (323, 345, 0, 320, 0.2),  # 박스 3 (기존 2번)
                (
                    300,
                    323,
                    0,
                    130,
                    0.0,
                ),  # TODO 01 : 박스 4 (새로 추가) - x범위 0~130: 횡단보도줄무늬(x≥139) 차단
                (
                    278,
                    300,
                    0,
                    130,
                    0.1,
                ),  # 박스 5 (기존 3번) - x범위 0~130: 횡단보도줄무늬(x≥139) 차단
            )
        else:
            self.rois = (
                (450, 480, 0, 320, 0.7),  # 박스 1 (기존 1번)
                (420, 450, 0, 320, 0.0),  # TODO 01 : 박스 2 (새로 추가)
                (390, 420, 0, 320, 0.2),  # 박스 3 (기존 2번)
                (360, 390, 0, 320, 0.0),  # TODO 01 : 박스 4 (새로 추가)
                (330, 360, 0, 320, 0.1),  # 박스 5 (기존 3번)
            )
        self.weight_sum = 1.0
    ...
    def add_horizontal_line(self, image):
        ...
        max_y = cv2.minMaxLoc(flip_binary)[-1][
            1
        ]  # extract the coordinates of the top-left point with a value of 255
    ...
    def add_vertical_line_far(self, image):
        ...
        x_0, y_0 = cv2.minMaxLoc(flip_binary)[
            -1
        ]  # extract the coordinates of the top-left point with a value of 255
        y_center = y_0 + 55
    ...
    def __call__(self, image, result_image):
        ...
        for roi in self.rois:
            ...
            contours = cv2.findContours(
                blob, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_L1
            )[
                -2
            ]  # find contours
            max_contour_area = self.get_area_max_contour(
                contours, 30
            ...
            if max_contour_area is not None:
                ...
                )  # draw the rectangle composed of the four points
...
def main():
    ...
    while running:
        ...
        )  # fill the top with black to avoid interference
