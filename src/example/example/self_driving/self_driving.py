...
class SelfDrivingNode(Node):
    def __init__(self, name):
        ...
        self.lane_detect_right.set_roi(
            (
                (338, 360, 320, 640, 0.7),
                (292, 315, 320, 640, 0.2),
                (248, 270, 320, 640, 0.1),
        ...
        self.double_press_window = (
            0.5  # 이 시간(초) 안에 두 번 누르면 더블프레스로 인정
        )
    ...
    def param_init(self):
        ...
        self.depth_park_enabled = (
            True  # True=뎁스 주차, False=기존 area/park_x arm-fire로 폴백
        )
        self.park_capture_dist = (
        ...
        self.turn_right_pass_dist = (
            200  # crosswalk_distance가 이 값 미만이면 '횡단보도 지나감'으로 판단
        )
        ...
        self.far_setpoint = (
            290  # 직선에서 라인 중앙일 때의 far 값(로그 실측 중앙값≈287). far가 이보다
        )
        #   작으면(우측 응시) 좌회전, 크면 우회전. 재획득 후 한쪽 치우치면 이 값 조정.
        self.far_recover_gain = (
            0.004  # far 오차→각속도 비례게인. 복구가 느리면 ↑, 좌우로 튀면 ↓
        )
        self.far_recover_speed = 0.12  # near 상실 중 저속 전진(라인을 다시 near ROI로 끌어옴). 빨라 이탈하면 ↓
        self.lane_lost_count = 0  # near가 연속으로 -1인 프레임 수(디바운스)
        self.lane_lost_frames = (
            4  # near가 이만큼 연속 -1일 때만 far 폴백. 코너 중 1~2프레임 순간 -1은 무시
        )
    ...
    def button_callback(self, msg):
        ...
        if now - self._last_press_time < self.double_press_window:
            ...
            self._pending_single_timer = threading.Timer(
                self.double_press_window, self.on_single_press
            )
    ...
    def on_single_press(self):
        ...
        if self.enter and not self.start:
            self.get_logger().info(
                "\033[1;32m%s\033[0m" % "START (single press) -> 주행 시작"
            )
            with self.lock:
                self.start = True
    ...
    def on_double_press(self):
        ...
        self.get_logger().info(
            "\033[1;33m%s\033[0m"
            % "DOUBLE press -> RESET to STANDBY (한 번 더 누르면 출발)"
        )
    ...
    def park_action_depth(self, fwd0, right0):
        ...
        self.get_logger().info(
            "\033[1;41mDEPTH PARK: forward %.2fm then strafe right %.2fm\033[0m"
            % (fwd_move, strafe_move)
        )
        if self.machine_type == "MentorPi_Mecanum":
            ...
            tw = Twist()
            tw.linear.y = -self.park_lateral_speed
            self.mecanum_pub.publish(tw)
    ...
    def publish_leds(self, c1, c2):
        ...
        self.get_logger().info(
            "\033[1;32mLED L=%s R=%s\033[0m" % (c1, c2)
        )  # [진단] LED 색 바뀔 때만
        msg = RGBStates()
        msg.states = [
    ...
    def main(self):
        while self.is_running:
            ...
            if self.start and self.parked:
                ...
                is_red = (
                    time.time() - self.red_last_seen_time
                ) < self.red_hold_time  # 모든 빨강 → 출발 막기(안전)
                red_close = (
                    time.time() - self.red_close_time
                ) < self.red_hold_time  # 가까운 빨강 → 정지 트리거
                ...
                if self.going_to_park and not self.start_park:
                    if self.depth_park_enabled:
                        ...
                        valid = (
                            0 < self.park_x
                            and 0.05 < self.park_depth_m <= self.park_capture_dist
                        )
                        if valid:
                            ...
                            if self.count_park >= self.park_capture_frames:
                                ...
                                self.get_logger().info(
                                    "\033[1;41m=== DEPTH PARK CAPTURE fwd=%.2f right=%.2f -> move fwd=%.2f strafe=%.2f ===\033[0m"
                                    % (
                                        fwd0,
                                        right0,
                                        max(0.0, fwd0 - self.park_standoff_fwd),
                                        right0 + self.park_standoff_right,
                                    )
                                )
                                self.mecanum_pub.publish(Twist())
                        ...
                        if 0 < self.park_x and self.park_area > self.park_min_area:
                            ...
                            self.count_park += 1
                            if (
                                self.count_park >= self.park_arm_frames
                        ...
                        if self.park_armed:
                            ...
                            advanced = (
                                0 < self.park_x
                                and self.park_x
                                >= self.park_arm_x + self.park_fire_advance
                            )
                            real_exit = (
                ...
                self.get_logger().info(
                    "\033[1;35mTURN? start_turn=%s lane_x=%d far=%d thr=%d stop=%s doing=%s go2park=%s start_park=%s\033[0m"
                    % (
                        self.start_turn,
                        lane_x,
                        lane_x_far,
                ...
                if not self.start_park and self.going_to_park and not self.stop:
                    ...
                    _, _, _, right_x = self.lane_detect_right(
                        binary_image, result_image
                    )
                    self.get_logger().info(
                        "\033[1;34mgoing_to_park right_x=%d (setpoint=%d)\033[0m"
                    ...
                    twist.linear.x = (
                        ...
                        if (self.park_x > 0 or self.park_armed)
                    ...
                    and not self.stop
                    and not self.doing_turn_right
                ):  # 우회전/주차 동작 중엔 차선추종 양보
                    self.lane_lost_count = (
                        0  # near로 라인을 잡았으니 미검출 카운터 리셋
                    )
                    if (
                    ...
                    self.get_logger().info(
                        "\033[1;32mDRIVE lin=%.2f ang=%.2f (start_turn=%s lane_x=%d)\033[0m"
                    ...
                    if self.lane_lost_count >= self.lane_lost_frames:
                        ...
                        ang = self.far_recover_gain * (
                            self.far_setpoint - lane_x_far
                        )  # far<setpoint(우측응시)→좌회전(+)
                        twist.angular.z = common.set_range(
                            ang, -self.angular_z_limit, self.angular_z_limit
                        )
                        self.get_logger().info(
                            "\033[1;33mLANE near-lost -> FAR fallback far=%d ang=%.2f cnt=%d\033[0m"
        ...
        if self.objects_info == []:  # If it is not recognized, reset the variable
            ...
            for i in self.objects_info:
                ...
                if class_name == "crosswalk":
                    ...
                    self.get_logger().info(
                        "\033[1;36mcrosswalk area=%d aspect=%.1f (min_area=%d min_asp=%.1f)\033[0m"
                        % (
                            cw_area,
                            aspect,
                            self.crosswalk_min_area,
                    ...
                    if (
                        cw_area >= self.crosswalk_min_area
                        and box_w
                        > self.crosswalk_min_aspect
                        * box_h  # 종횡비: 가로가 세로의 N배 이상
                    ...
                    if (
                        right_area >= self.right_min_area
                        and not self.going_to_park
                        and not self.start_park
                        and not self.parked
                    ):
                    ...
                    if class_name == "red":
                        self.red_last_seen_time = (
                            time.time()
                        )  # [신호등] 빨강 신선도 갱신(모든 빨강 → 출발 막기)
                        red_area = (i.box[2] - i.box[0]) * (i.box[3] - i.box[1])
                        self.get_logger().info(
                            "\033[1;31mred area=%d (min=%d)\033[0m"
                            % (red_area, self.red_min_area)
                        )
                        if (
                            red_area >= self.red_min_area
                        ):  # 가까운 빨강 → 정지 트리거 갱신
