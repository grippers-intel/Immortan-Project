        ...
        while self.is_running:
            ...
            if self.start and self.parked:
                ...
                if not self.start_park and self.going_to_park and not self.stop:
                    ...
                    self.pid.clear()
                    # [추가] stop이 아닌데 lane_x를 못 찾은 경우 → 무한 정지 방지용 fallback
                    if (
                        not self.stop
                        and not self.doing_turn_right
                        and not self.start_park
                        ...
                        if lane_x < 0:
                            self.lane_lost_count = (
                                getattr(self, "lane_lost_count", 0) + 1
                            )
                        else:
                            self.lane_lost_count = 0
                        fallback = Twist()
                        # 너무 오래 잃어버리지 않았다면: 최근 조향 방향을 유지하며 서행 (완전 정지 방지)
                        if self.lane_lost_count < 30:  # ≈0.9초 (30fps 가정)
                            fallback.linear.x = self.slow_down_speed
                            fallback.angular.z = self.last_angular_z  # 아래 참고
                        else:
                            fallback.linear.x = 0.0
                            fallback.angular.z = 0.0
                        self.mecanum_pub.publish(fallback)
